...
function rmc_validate_cron_config() {
    var is_valid = true;
    var first_invalid_element_jq = null;
    var enable_task_type = jQuery("#rmc_task_type");
    if(enable_task_type.val() == 'automatic') {
        var sub_jq = jQuery("#rmc_task_first_execution");        
        if(sub_jq.val() == "") {
            is_valid = false;
            if(!first_invalid_element_jq)
                first_invalid_element_jq = sub_jq;
        ...
        var sub_jq = jQuery("#rm-task-schedule");        
        if(sub_jq.val() =="") {
            is_valid = false;
            if(!first_invalid_element_jq)
                first_invalid_element_jq = sub_jq;
    ...
    if(first_invalid_element_jq)
        first_invalid_element_jq.focus();
    if(is_valid)
        jQuery( "form#rmc_task_settings_form" ).submit();
    return is_valid;
}
...
jQuery('#rmc_task_type').change(function(){

    var type = jQuery(this).val();
    if(type == 'automatic'){
        jQuery('#rmc_enable_crons').show();
        jQuery('#rmc_task_first_execution, #rm-task-schedule').prop('required',true);
    }
    else{
        jQuery('div#rmc_enable_crons').hide();
        jQuery('#rmc_task_first_execution, #rm-task-schedule').prop('required',false);
    }
});