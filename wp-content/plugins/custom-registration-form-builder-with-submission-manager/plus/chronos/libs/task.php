...
class RM_Chronos_Task {
        ...
        foreach($this->actions as $action) {
            switch($action) {
                ...
                case RM_Chronos_Action_Interface::ACTION_TYPE_SEND_EMAIL:
                    ...
                    break;
                /*
                case RM_Chronos_Action_Interface::ACTION_TYPE_ASSIGN_ROLE:
                    if(isset($data->subs) && is_array($data->subs) && defined('REGMAGIC_ADDON')) {
                        $this->action_assign_user_roles($data->subs);
                    }
                    break;
                */
    ...
    public function action_assign_user_roles($subs) {
        $roles = isset($this->meta['user_roles']) ? $this->meta['user_roles'] : array();
        $prev_proc_subs = RM_Chronos_Toolkit::get_already_processed_subs($this->task_id, RM_Chronos_Action_Interface::ACTION_TYPE_ASSIGN_ROLE);
        $prev_proc_subs = $prev_proc_subs->sub_ids;
        $new_proc_subs = array();
        if(empty($roles))
            return false;
        ...
        foreach($subs as $sub) {
            if(in_array($sub->submission_id,$prev_proc_subs))
                continue;
            $params = array('sub_id' => $sub->submission_id, 'form_id' => $this->form_id);
            $user = get_user_by('email', $sub->user_email);
        ...
        RM_Chronos_Toolkit::update_processed_subs($this->task_id, RM_Chronos_Action_Interface::ACTION_TYPE_ASSIGN_ROLE, $new_proc_subs);
    }
