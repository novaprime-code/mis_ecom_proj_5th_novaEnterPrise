...
class RM_Chronos_Query_Builder {
    ...
    protected function get_submissions($prime_joiner) {
        ...
        */
        if(is_array($fsearch_query) && !empty($fsearch_query)) {
            foreach($fsearch_query as $qr) {
                $query .= " $prime_joiner st.submission_id IN ($qr)";
            }
        }
        ...
        if(isset($this->sub_query_arg['field_search'])) {
            ...
            foreach($this->sub_query_arg['field_search'] as $field_search) {
                ...
                if($ival_q != "") {
                    ...
                    $temp_query = "(`field_id` = $field_id AND ($ival_q))";
                } else {
                    $temp_query = "";
                }
                
                if($temp_query != "") {
                    $ext_query .= " WHERE `form_id` = {$this->form_id} AND $temp_query";
                } else {
                    $ext_query = "";
                }
                
                array_push($query_arr, $ext_query);
            }
        }
        /*
        if($ext_query != "")
