    ...
    public function create_mobile_field() {
        ...
        if($this->field_options->format_type=='international'){
            ...
            if(!empty($this->field_options->sync_country) && $this->field_options->country_field!='not_found'){
              ...
              if($rm_country_field->field_type=='Address'){
                  if($embed){
                      ...
                      . " var selected_value= jQuery(this).val(); var country=rm_get_country_code_by_name(rm_country_list,selected_value); jQuery('[id=" . wp_kses_post($this->opts['id']) . "]').intlTelInput('setCountry',country); ".wp_kses_post($force_match_js)." "
                      . "})});</script>";
                  }
                  else
                  {
                   ...
                   if($embed){
                       echo "<script>jQuery(document).ready(function(){jQuery('[name=\'".wp_kses_post($country_field_name)."\']').change(function(){" 
                      . " var selected_value= jQuery(this).val(); var index= selected_value.search(/\[[A-Z]{2}\]/i); if(index>=0) { var country= selected_value.substr(index+1,2).toLowerCase(); jQuery('[id=" . wp_kses_post($this->opts['id']) . "]').intlTelInput('setCountry',country); ".wp_kses_post($force_match_js)."} "
                      . "})});</script>";
                   }
                   else
                   {    
            ...
            if($ca_state_type=='america'){
                ...
                if(!empty($this->field_options->country_match)){
                    if($embed){
                         echo "<script>jQuery(document).ready(function(){setTimeout(function(){ var el= jQuery('#" . wp_kses_post($this->opts['id']) . "'); var form= el.closest('form'); form.submit(function(event){if(!el.intlTelInput('isValidNumber')){ return rm_toggle_tel_wc_error(false,el,'".wp_kses_post($error_msg)."'); return false; } else { return rm_toggle_tel_wc_error(false,el,''); } el.val(el.intlTelInput('getNumber'))}); el.intlTelInput({initialCountry:'us',preferredCountries:".wp_kses_post($preferred_countries).",".wp_kses_post($placeholder_sett)."}); el.closest('.rminput').find('.selected-flag').addClass('disable-flag'); }, 3000); });</script>";
                    }
                    else
                    {
                ...
                {   
                     if($embed){
                         echo "<script>jQuery(document).ready(function(){setTimeout(function(){ var el= jQuery('#" . wp_kses_post($this->opts['id']) . "'); var form= el.closest('form');  form.submit(function(event){if(!el.intlTelInput('isValidNumber')){ return rm_toggle_tel_wc_error(false,el,'".wp_kses_post($error_msg)."'); return false; } else { return rm_toggle_tel_wc_error(false,el,''); } el.val(el.intlTelInput('getNumber'))}); el.intlTelInput({initialCountry:'us',preferredCountries:".wp_kses_post($preferred_countries).",".wp_kses_post($placeholder_sett)."}); }, 3000); });</script>"; 
                     }
                     else
                     {
                ...
                if($embed){
                    echo "<script>jQuery(document).ready(function(){setTimeout(function(){ var el= jQuery('#" . wp_kses_post($this->opts['id']) . "'); var form= el.closest('form'); form.submit(function(event){if(!el.intlTelInput('isValidNumber')){ return rm_toggle_tel_wc_error(false,el,'".wp_kses_post($error_msg)."'); return false; } else { return rm_toggle_tel_wc_error(false,el,''); } el.val(el.intlTelInput('getNumber'))}); el.intlTelInput(".wp_kses_post($tel_params)."); }, 3000);});</script>";   
                }
                else
                {    
            ...
            $meta_value = '';
            if(is_user_logged_in()){ // Check for existing meta values
                if($this->field_options->field_user_profile == 'define_new_user_meta')
				    $meta_value = get_user_meta(get_current_user_id(),$this->field_options->field_meta_add,true);
                if($this->field_options->field_user_profile == 'existing_user_meta')
				    $meta_value = get_user_meta(get_current_user_id(),$this->field_options->existing_user_meta_key,true);
            }
            $this->opts['value'] = $meta_value;
            ...
            $tel_params.= 'utilsScript:"'.$util_js.'"}';
            if($embed){
                echo "<script>jQuery(document).ready(function(){jQuery('#" . wp_kses_post($this->opts['id']) . "').intlTelInput(".wp_kses_post($tel_params).");});</script>";
                echo "<script>jQuery(document).ready(function(){var el= jQuery('#" . wp_kses_post($this->opts['id']) . "'); var el= jQuery('#" . wp_kses_post($this->opts['id']) . "');  var form= el.closest('.rmagic-form'); form.submit(function(event){el.prop('pattern',''); if(!el.intlTelInput('isValidNumber')){ return rm_toggle_tel_wc_error(false,el,'".wp_kses_post($error_msg)."'); return false; } else {return rm_toggle_tel_wc_error(true,el,''); } el.val(el.intlTelInput('getNumber'));});});</script>";
            }
            else{
